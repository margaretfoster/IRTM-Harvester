...
setwd("~/Dropbox/IRTM-Harvester/code/")

if(Sys.info()['user']== 'Promachos'){## MJF 2023 Laptop
  print("Hi Margaret!")
  dataPath <- "~/Dropbox/Interventions/Interventions-Event-Data/"
  thetasPath  <-"./Results/"
  print("Paths loaded")
}


load(paste0(thetasPath, "gridyear-Thetas-All.Rds")) ## loads Grids and Theta summaries

source("coordsToPrioGrid.R") ## helper to convert long-lat to PRIO grid system

## Data:

## grid.meta <- read.csv(paste0(dataPath,"PRIO-grid-yearly.csv")) ## yearly grid coviariates; if desired

acled <-  read.csv(paste0(dataPath, "acled_africa.csv"), sep=";") ## 334,188 -- has PRIO-field
...
acled[which(acled$disorder_type=="Political violence; Demonstrations"), c("disorder_type")] <- "Political violence"
...
## 12/13/23: Research design update 
## Modeling types of violence separately
## I have a thematic clustering scheme; but first
## run for each interaction type
## Then, if we want, can compare for the bundled type 
...
ggsave(d_cors.plot, file="correlations_demonstrations_meanT1.png")
...
ggsave(d_cors_p2, file="MeanTheta1_demonstrations_cors_byr.png")
...
v_cors_p3 <- ggplot(v_cors_2[which(v_cors_2$EventType==event_focus),],
                      aes(y= Correlation, 
                          ...
                          fill=Correlation)) +
  geom_bar(stat="identity") +
  scale_fill_gradient2(high="red", low="darkblue", midpoint=0) +
  labs(x="\nCountry", 
