...
load(paste0(abdata, "dfWithABEthnicGroupsR4.Rdata")) ## Loads a df with names

## IRT-M Data for Thetas
load(paste0(dataPath, paste0(country, country, "_R4_direct_THETA.RData"))) ## Country-round Thetas from IRT-M
load(paste0(abdata, country, "R4RawAB.Rds")) ## Original survey data, made by breakdownABRounds.R
...
geoThetas <- ABThetasR4[,writeout]
geoThetas$country <- country
geoThetas$round <- 4

save(geoThetas,
     file=paste0(thetasPath,
                 country, "R4geolocatedThetas.Rds" ))
