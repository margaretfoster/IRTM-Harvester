...
## Tailored to Kenya Round 6 (based on Nigeria script)
...
load(paste0(dataPath, "KenyaKenya_R6_direct_THETA.RData")) ## Thetas
load(paste0(abdata, "kenyaR6RawAB.Rdata")) ## made by breakdownABRounds.R

###########################
## ROUND 5
###########################

ABRound <- kenyaR6 ## name change for more modular code

rm(kenyaR6)

dim(ABRound)## 2397 x 360
dim(THETA)
...
#################
## Ethnic Groups
## Merged R6, ethnic grup question is q87
...
#save(egsummary, file="KenyaR6egsummary.Rdata")
...
#save(regsummary, file="kenyar6regionsummary.Rdata")
...
geoThetasR6$country <- "Kenya"
geoThetasR6$round <- 6

#save(geoThetasR6,
#     file=paste0(thetasPath,
#                 "KenyaR6geolocatedThetas.Rdata" ))


#######
colnames(ABThetasR6)[colnames(ABThetasR6)=="V2"] <- "groupname"
ABThetasR6$regionnames <- as.factor(ABThetasR6$regionnames)
...
R6map$round <- 6
#save(R6map, file="KenyaR6mapdat.Rdata")
...
ggplot(ABThetasR6) + 
  ...
  ggtitle('Posterior Distribution of Group Mean Theta 1',
          subtitle='Kenya, Round 6 (2014), By Ethnic Group') + 
...
ggsave('Kenya_r6-theta1_group_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_r6-theta2_group_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_r6-theta3_group_mean_posterior.png',
       width = 10, height=5)
...
ggplot(ABThetasR6) + 
  ...
  ggtitle('Posterior Distribution of Region Mean Theta 1',
          subtitle='Kenya, Round 6 (2014), By Region') + 
  xlab(TeX('$\\theta_1$')) + ylab('')+
  labs(caption= print(Sys.Date())) + 
...
ggsave('Kenya_r6-theta1_region_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_r6-theta2_region_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_r6-theta3_region_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_R6-theta1_urbrur_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_R6-theta2_urbrur_mean_posterior.png',
       width = 10, height=5)
...
ggsave('Kenya_R6-theta3_urbrur_mean_posterior.png',
       width = 10, height=5)
